...
const SimpleArticle = memo(props => {
    const {
        title,
        publishedTime,
        body,
    ...
    const [isCollect2, setIsCollect] = useState(false);
    useEffect(() => {
            ...
            {timeFlag && (
                <div className="time">
                    <div className="timeSec">
                        {new Date(parseInt(publishedTime, 0) * 1000)
                            .toLocaleString()
                            .replace(/:\d{1,2}$/, " ")}
                    </div>
                    <div className="collectIcon" onClick={toggleCollect}>
                        {isCollect2 ? (
                            <Fragment>
                                <svg
                                    t="1594290153620"
