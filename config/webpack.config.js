...
module.exports = function (webpackEnv) {
  ...
  const getStyleLoaders = (cssOptions, preProcessor) => {
    const loaders = [
      ...
      isEnvProduction && {
        ...
        // Options for PostCSS as we reference these options twice Adds vendor prefixing
        // based on your specified browser support in package.json
  ...
  return {
    ...
    // "root" imports that are included in JS bundle.
    // entry: {
    ...
    //   // require.resolve('webpack/hot/dev-server'), isEnvDevelopment &&
    //   // require.resolve('react-dev-utils/webpackHotDevClient'), // Finally, this is
    //   // your app's code: paths.appIndexJs, We include the app code last so that if
    //   // there is a runtime error during initialization, it doesn't blow up the
    //   // WebpackDevServer client, and changing JS code would still trigger a refresh.
    //   pc: [
    //     isEnvDevelopment && require.resolve('react-dev-utils/webpackHotDevClient'),
    ...
    output: {
      ...
      // We inferred the "public path" (such as / or /my-project) from homepage. We
      // use "/" in development.
    ...
    optimization: {
        ...
        new TerserPlugin({
          terserOptions: {
            parse: {
              ...
              // minfication steps that turns valid ecma 5 code into invalid ecma 5 code. This
              // is why the 'compress' and 'output' sections only apply transformations that
              // are ecma 5 safe https://github.com/facebook/create-react-app/pull/4234
              ecma: 8
            },
            compress: {
              ecma: 5,
              ...
              // https://github.com/facebook/create-react-app/issues/2376 Pending further
              // investigation: https://github.com/mishoo/UglifyJS2/issues/2011
              comparisons: false,
              // Disabled because of an issue with Terser breaking valid code:
              // https://github.com/facebook/create-react-app/issues/5250 Pending futher
              // investigation: https://github.com/terser-js/terser/issues/120
              inline: 2
            },
            mangle: {
              safari10: true
          ...
          cssProcessorOptions: {
            ...
            map: shouldUseSourceMap
              ? {
                ...
                inline: false,
                // `annotation: true` appends the sourceMappingURL to the end of the css file,
      ...
      // https://medium.com/webpack/webpack-4-code-splitting-chunk-graph-and-the-split
      // c hunks-optimization-be739a861366
      splitChunks: {
        chunks: 'all',
        name: false
      },
    ...
    resolve: {
      ...
      // https://github.com/facebook/create-react-app/issues/290 `web` extension
      // prefixes have been added for better support for React Native Web.
      extensions: paths
      ...
      plugins: [
        // Adds support for installing with Plug'n'Play, leading to faster installs and
        // adding guards against forgotten dependencies and such.
        PnpWebpackPlugin,
    ...
    resolveLoader: {
      plugins: [// Also related to Plug'n'Play, but this time it tells Webpack to load its
        // loaders from the current package.
      ...
      rules: [
        ...
        // JS.
        {
          ...
          oneOf: [
            ...
            // Process application JS with Babel. The preset includes JSX, Flow, TypeScript,
            // and some ESnext features.
            {
            ...
            // Process any JS outside of the app with Babel. Unlike the application JS, we
            // only compile the standard ES features.
            {
              test: /\.(js|mjs)$/,
              ...
              options: {
                ...
                // If an error happens in a package, it's possible to be because it was
                // compiled. Thus, we don't want the browser debugger to show the original code.
                // Instead, the code being evaluated would be much more helpful.
                sourceMaps: false
              }
            },
            ...
            // modules that inject <style> tags. In production, we use MiniCSSExtractPlugin
            // to extract that CSS to a file, but in development "style" loader enables hot
            // editing of CSS. By default we support CSS Modules with the extension
            // .module.css
            {
              test: cssRegex,
            ...
            // Opt-in support for SASS (using .scss or .sass extensions). By default we
            // support SASS Modules with the extensions .module.scss or .module.sass
            {
              test: sassRegex,
            ...
            // it will catch all modules that fall through the other loaders.
            {
              ...
              // that would otherwise be processed through "file" loader. Also exclude `html`
              // and `json` extensions so they get processed by webpacks internal loaders.
              exclude: [
    ...
    plugins: [
      ...
      //   }
      //   : undefined)),
      // Inlines the webpack runtime script. This script is too small to warrant a
      // network request.
      ...
      // pathname of that URL. In development, this will be an empty string.
      new InterpolateHtmlPlugin(HtmlWebpackPlugin, env.raw),
      // This gives some necessary context to module not found errors, such as the
      // requesting resource.
      ...
      // absolutely essential that NODE_ENV is set to production during a production
      // build. Otherwise React will be compiled in the very slow development mode.
      ...
      // https://github.com/facebook/create-react-app/issues/186
      ...
      isEnvProduction && new WorkboxWebpackPlugin.GenerateSW({
        ...
        navigateFallbackBlacklist: [
          ...
          new RegExp('^/_'),
          // Exclude URLs containing a dot, as they're likely a resource in public/ and
          // not a SPA route
          new RegExp('/[^/]+\\.[^/]+$')
        ]
      }),
    ...
    // Turn off performance processing because we utilize our own hints via the
    // FileSizeReporter
    performance: false
  };
};
